...
class LinkPreviewModule {
  constructor() {
    ...
    settings.on(`changed.${SettingIds.LINK_PREVIEW_HIDE_ALL}`, () => this.setup());
  ...
  setup() {
    this.showPreview = settings.get(SettingIds.LINK_PREVIEW);
    this.hideAll = settings.get(SettingIds.LINK_PREVIEW_HIDE_ALL);
    this.updateProcessorSettings();

    if (this.showPreview || this.hideAll) {
